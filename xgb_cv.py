#!/usr/bin/env python3
"""
XGBoost (XGBClassifier) with 10-fold Stratified CV + SMOTE-Tomek (k=5), reporting Accuracy, Precision (macro), Recall (macro), F1 (macro).

Usage:
  python xgb_cv.py --data path/to/data.csv --target TARGET_COLUMN
...
def build_model():
    ...
    clf = XGBClassifier(n_estimators=100, learning_rate=0.3, max_depth=6, subsample=1.0, colsample_bytree=1.0, reg_lambda=1.0, objective='binary:logistic', eval_metric='logloss', tree_method='hist', random_state=42)
    return clf
...
def main():
    ...
    print("===== XGBoost (XGBClassifier) (10-fold CV) =====")
